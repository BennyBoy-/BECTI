...
//todo req_toai for indep salvager from client!
	if (_req_toai) then { //--- Send a purchase request to the client if the target is not an AI.
		if (isPlayer leader _req_buyer && typeName _req_target != "SIDE") then {
			if (_req_buyer != _req_target) then {[_req_seed, _req_classname, _req_target, _factory] remoteExec ["CTI_PVF_CLI_OnPurchaseDelegationStart", leader _req_buyer]};
		};
		...
		if (typeName _req_target != "SIDE") then {
			...
			[_req_seed, _req_classname, _req_buyer, _factory, _req_veh_infos] remoteExec ["CTI_PVF_CLI_OnPurchaseOrderReceived", leader _req_target];
