...
//--- Find AT Lock weapons.
{
	...
	
	{
		...
		if (_ammo != "") then {
			...
			if (getNumber(configFile >> "CfgAmmo" >> _ammo >> "irLock") == 1 && getNumber(configFile >> "CfgAmmo" >> _ammo >> "airLock") == 0 && configName(inheritsFrom(configFile >> "CfgAmmo" >> _ammo)) == "MissileBase") then {_remove = true; _magazines_remove = _magazines_remove + [_x]};
		};
